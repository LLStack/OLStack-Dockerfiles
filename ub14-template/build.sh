...
build_image(){
    if [ -z "${1}" ] || [ -z "${2}" ]; then
        ...
        echo "${1} ${2}"
        docker build . --tag ${BUILDER}/${REPO}:ols${1}-php${2} --build-arg OLS_VERSION=${1} --build-arg PHP_VERSION=${2}
    fi    
}

test_image(){
    ID=$(docker run -d ${BUILDER}/${REPO}:ols${1}-php${2})
...
push_image(){
    if [ ! -z "${PUSH}" ]; then
        ...
        docker ${CONFIG} push ${BUILDER}/${REPO}:ols${1}-php${2}
        if [ ! -z "${TAG}" ]; then
            docker tag ${BUILDER}/${REPO}:ols${1}-php${2} ${BUILDER}/${REPO}:${3}
